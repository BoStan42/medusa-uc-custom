...
import { useAdminAddPublishableKeySalesChannelsBatch, useAdminCreatePublishableApiKey } from 'medusa-react';
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h5 className="inter-base-semibold text-grey-90 pb-1">{t('pages-sales-channels', 'Sales channels')}</h5>
...
function CreatePublishableKey(props: CreatePublishableKeyProps) {
  ...
  const onSubmit = async () => {
    try {
      ...
      notification(
        t('pages-error', 'Error'),
        t('pages-failed-to-create-a-new-api-key', 'Failed to create a new API key'),
        'error',
      );
    }
  };
    ...
    <FocusModal>
      ...
      <FocusModal.Main className="no-scrollbar flex w-full justify-center">
        <div className="medium:w-7/12 large:w-6/12 small:w-4/5 my-16 max-w-[700px]">
          <h1 className="inter-xlarge-semibold text-grey-90 pb-8">{t('pages-create-api-key', 'Create API Key')}</h1>

...
const DraftOrderDetails = () => {
  ...
  const [deletePromptData, setDeletePromptData] = useState<DeletePromptData>(initDeleteState);
  const [addressModal, setAddressModal] = useState<null | {
    address: Address;
    type: AddressType;
  }>(null);

  const [showMarkAsPaidConfirmation, setShowAsPaidConfirmation] = useState(false);
  ...
  const onMarkAsPaidConfirm = async () => {
    try {
      ...
      notification(
        ...
        t('draft-orders-successfully-mark-as-paid', 'Successfully mark as paid'),
        'success',
      );
    } catch (err) {
  ...
  const handleDeleteOrder = async () => {
    return cancelOrder.mutate(void {}, {
      onSuccess: () =>
        notification(
          t('draft-orders-success', 'Success'),
          t('draft-orders-successfully-canceled-order', 'Successfully canceled order'),
          'success',
        ),
    ...
    <div>
        ...
        <div className="flex space-x-4">
          <div className="flex h-full w-full flex-col">
            ...
            <BodyCard
              ...
              title={t('on-mark-as-paid-confirm-order-id', 'Order #{{display_id}}', {
              ...
              customActionable={
                draft_order?.status === 'completed' && (
                  <Button
                    variant="secondary"
                    size="small"
                    onClick={() => navigate(`/a/orders/${draft_order.order_id}`)}
                  >
            ...
            >
              <div className="mt-6 flex space-x-6 divide-x">
                <div className="flex flex-col">
                  <div className="inter-smaller-regular text-grey-50 mb-1">{t('draft-orders-email', 'Email')}</div>
                  <div>{cart?.email}</div>
                </div>
                <div className="flex flex-col pl-6">
                  <div className="inter-smaller-regular text-grey-50 mb-1">{t('draft-orders-phone', 'Phone')}</div>
            ...
            <BodyCard className={'mb-4 h-auto min-h-0 w-full'} title={t('draft-orders-shipping', 'Shipping')}>
              <div className="mt-6">
                  ...
                  <div className="flex flex-col" key={method.id}>
                    ...
                    <div className="bg-grey-5 mt-8 flex h-full min-h-[100px] flex-col px-3 py-2">
                      <span className="inter-base-semibold">
                        ...
                        <span className="text-grey-50 inter-base-regular">{t('draft-orders-1-item', '(1 item)')}</span>
                      </span>
              ...
              <div className="mt-6">
                <div className="flex w-full items-center space-x-4">
                  <div className="flex h-[40px] w-[40px] ">
                    <Avatar user={cart?.customer} font="inter-large-semibold" color="bg-grey-80" />
                  </div>
                  <div>
                ...
                <div className="mt-6 flex space-x-6 divide-x">
                  <div className="flex flex-col">
                    <div className="inter-small-regular text-grey-50 mb-1">{t('draft-orders-contact', 'Contact')}</div>
      ...
      {deletePromptData.show && (
        <DeletePrompt
          ...
          heading={t('draft-orders-remove-resource-heading', 'Remove {{resource}}', {
            resource: deletePromptData?.resource,
          })}
          successText={t('draft-orders-remove-resource-success-text', '{{resource}} has been removed', {

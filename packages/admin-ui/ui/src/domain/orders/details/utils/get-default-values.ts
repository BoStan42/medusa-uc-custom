...
const getDefaultShippingAddressValues = (order: Order): Subset<AddressPayload> => {
  ...
  const keys = Object.keys(order.shipping_address).map(k => k) as (keyof AddressPayload)[];
...
const getReturnableItemsValues = (order: Order) => {
  ...
  const returnableItems = order.returns
    .filter(r => r.status === 'requested')
    .reduce((acc, r) => (acc = [...acc, ...r.items]), [] as Return['items']);

  const items = order.items.filter(i => returnableItems.find(r => r.item_id === i.id));
...
const getReceiveableItemsValues = (order: Order, returnRequest: Return): Subset<ItemsToReceiveFormType> => {
  ...
  const returnItems = {
    items: returnableItems?.items?.reduce((acc, item) => {
      ...
      const indexOfRequestedItem = returnRequest.items.findIndex(i => i.item_id === item.item_id);

      if (item.item_id && indexOfRequestedItem > -1) {
        const requestedItem = returnRequest.items[indexOfRequestedItem];

        const adjustedQuantity = requestedItem.requested_quantity - requestedItem.received_quantity;
...
export const getDefaultReceiveReturnValues = (order: Order, returnRequest: Return): Subset<ReceiveReturnFormType> => {
  return {
    receive_items: getReceiveableItemsValues(order, returnRequest),
  };
};

export const getDefaultClaimValues = (order: Order): Subset<CreateClaimFormType> => {
  return {

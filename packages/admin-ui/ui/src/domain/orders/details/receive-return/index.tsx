...
export const ReceiveReturnMenu = ({ order, returnRequest, onClose }: Props) => {
  ...
  const [inventoryMap, setInventoryMap] = useState<Map<string, InventoryLevelDTO[]>>(new Map());
  ...
  }, [client.admin.variants, isLocationFulfillmentEnabled, itemMap, returnRequest.items]);
  ...
  useEffect(() => {
    if (isLocationFulfillmentEnabled && stock_locations?.length) {
      const location = stock_locations.find((sl: StockLocationDTO) => sl.id === returnRequest.location_id);
      if (location) {
        setSelectedLocation({
          value: location.id,
          label: location.name,
  ...
  const isRefundedClaim = useMemo(() => {
    if (returnRequest.claim_order_id) {
      const claim = order.claims.find(c => c.id === returnRequest.claim_order_id);
      if (!claim) {
        return false;
      }
        ...
        <form onSubmit={onSubmit}>
          <Modal.Content>
            <div className="gap-y-large flex flex-col">
              <ItemsToReceiveForm order={order} form={nestedForm(form, 'receive_items')} />

              {isLocationFulfillmentEnabled && (
                <div className="mb-8">
                  <h3 className="inter-base-semibold ">{t('receive-return-location', 'Location')}</h3>
                  <p className="inter-base-regular text-grey-50">
                    {t('receive-return-choose-location', 'Choose which location you want to return the items to.')}
                  </p>
                  {isLoadingLocations ? (
                    <Spinner />
                  ) : (
              ...
              {!isSwapOrRefundedClaim && (
                <ReceiveReturnSummary form={form} order={order} returnRequest={returnRequest} />
              )}
            </div>
          </Modal.Content>
          <Modal.Footer>

...
const GiftCardDetails = () => {
  ...
  const { state: editState, open: openEdit, close: closeEdit } = useToggleState();

  const { state: balanceState, open: openBalance, close: closeBalance } = useToggleState();
        ...
        <>
          <div className="gap-y-xsmall flex flex-col">
            {getWidgets('custom_gift_card.before').map((w, i) => {
              return <WidgetContainer key={i} widget={w} entity={giftCard} injectionZone="custom_gift_card.before" />;
            })}
            ...
            <BodyCard
              ...
              status={
                <StatusSelector
                  ...
                  activeState={'Active'}
                  draftState={'Disable'}
                  onChange={() => updateStatus({ is_disabled: !giftCard.is_disabled })}
                />
              }
              actionables={actions}
            >
              <div className="flex justify-between">
                <div className="flex space-x-6 divide-x">
                  ...
                  <div className="flex flex-col pl-6">
                    <div className="inter-smaller-regular text-grey-50 mb-1">{t('details-balance', 'Balance')}</div>
                    <div>
                      {formatAmountWithSymbol({
                  ...
                  <div className="flex flex-col pl-6">
                    <div className="inter-smaller-regular text-grey-50 mb-1">{t('details-region', 'Region')}</div>
                  ...
                  {giftCard.ends_at && (
                    <div className="flex flex-col pl-6">
                      ...
                      <div>{moment(giftCard.ends_at).format('DD MMM YYYY')}</div>
                    </div>
                  )}
                  <div className="flex flex-col pl-6">
                    <div className="inter-smaller-regular text-grey-50 mb-1">{t('details-created', 'Created')}</div>
            ...
            {getWidgets('custom_gift_card.after').map((w, i) => {
              return <WidgetContainer key={i} widget={w} entity={giftCard} injectionZone="custom_gift_card.after" />;
            })}

            <RawJSON data={giftCard} title={t('details-raw-gift-card', 'Raw gift card')} />
          </div>

          <UpdateBalanceModal giftCard={giftCard} onClose={closeBalance} open={balanceState} />

          <EditGiftCardModal onClose={closeEdit} open={editState} giftCard={giftCard} />
        </>
      )}
    </div>
  );

...
function CustomerGroupCustomersList(props: CustomerGroupCustomersListProps) {
  ...
  const { q, queryObject, paginate, setQuery } = useQueryFilters(defaultQueryProps);
  ...
  const { mutate: removeCustomers } = useAdminRemoveCustomersFromCustomerGroup(groupId);

  // list of currently selected customers of a group
  const [selectedCustomerIds, setSelectedCustomerIds] = useState(customers.map(c => c.id));
  ...
  return (
    <BodyCard title={t('groups-customers', 'Customers')} actionables={actions} className="min-h-[756px] w-full">
      {showCustomersModal && (
        <EditCustomersTable
...
function CustomerGroupDetailsHeader(props: CustomerGroupDetailsHeaderProps) {
  ...
  const { mutate: deleteGroup } = useAdminDeleteCustomerGroup(props.customerGroup.id);
    ...
    <>
      ...
      {showDeleteConfirmation && (
        <DeletePrompt
          ...
          text={t('groups-confirm-delete-customer-group', 'Are you sure you want to delete this customer group?')}
        />
      )}
      <CustomerGroupModal open={state} onClose={close} customerGroup={props.customerGroup} />
    </>
  );
}
    ...
    <div className="-mt-4 pb-4">
      ...
      <div className="gap-y-xsmall flex flex-col">
        {getWidgets('customer_group.details.before').map((w, i) => {
          return (
            <WidgetContainer key={i} entity={customer_group} injectionZone="customer_group.details.before" widget={w} />
          );
        })}
        ...
        {getWidgets('customer_group.details.after').map((w, i) => {
          return (
            <WidgetContainer key={i} entity={customer_group} injectionZone="customer_group.details.after" widget={w} />
          );
        })}

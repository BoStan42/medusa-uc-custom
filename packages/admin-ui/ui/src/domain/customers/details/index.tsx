    ...
    <div>
      ...
      <div className="gap-y-xsmall flex flex-col">
        {getWidgets('customer.details.before').map((w, i) => {
          return <WidgetContainer key={i} entity={customer} injectionZone="customer.details.before" widget={w} />;
        })}
        ...
        <Section>
          <div className="flex w-full items-start justify-between">
            <div className="gap-x-base flex w-full items-center">
              <div className="h-[64px] w-[64px]">
                <Avatar user={customer} font="inter-2xlarge-semibold w-full h-full" color="bg-fuschia-40" />
              </div>
              <div className="flex grow flex-col">
                <h1 className="inter-xlarge-semibold text-grey-90 max-w-[50%] truncate">{customerName()}</h1>
                <h3 className="inter-small-regular text-grey-50">{customer.email}</h3>
              </div>
            </div>
          ...
          <div className="mt-6 flex space-x-6 divide-x">
            <div className="flex flex-col">
              <div className="inter-smaller-regular text-grey-50 mb-1">{t('details-first-seen', 'First seen')}</div>
            ...
            <div className="flex flex-col pl-6">
              <div className="inter-smaller-regular text-grey-50 mb-1">{t('details-orders', 'Orders')}</div>
              <div>{customer?.orders?.length}</div>
            </div>
            <div className="h-100 flex flex-col pl-6">
              <div className="inter-smaller-regular text-grey-50 mb-1">{t('details-user', 'User')}</div>
            ...
            <FormattedAddress title={'Billing'} addr={customer.billing_address} />
            {customer?.shipping_addresses?.map(address => (
              <FormattedAddress key={address.id} title={'Shipping'} addr={address} />
            ))}
          </div>
        </Section>
        ...
        {getWidgets('customer.details.after').map((w, i) => {
          return <WidgetContainer key={i} entity={customer} injectionZone="customer.details.after" widget={w} />;
        })}
      ...
      {showEdit && customer && (
        <EditCustomerModal customer={customer} refetchCustomer={fetchCustomer} handleClose={() => setShowEdit(false)} />
      )}
    </div>
  );
};

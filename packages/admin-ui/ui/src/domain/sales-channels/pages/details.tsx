...
function ListIndicator(props: ListIndicatorProps) {
  ...
  return (
    ...
    >
      {isActive && <div className="bg-violet-60 rounded-circle h-[10px] w-[10px]" />}
    </div>
  );
}
...
function SalesChannelTile(props: SalesChannelTileProps) {
  ...
  return (
    <div
      onClick={onClick}
      className={clsx('mb-2 flex h-[83px] cursor-pointer justify-between rounded-lg border p-4', {
    ...
    >
      <div className="flex gap-2 overflow-hidden">
        ...
        <div className="block overflow-hidden truncate">
          <h3 className="text-grey-90 mb-1 font-semibold leading-5">{salesChannel.name}</h3>
          <span className="text-small text-grey-50 ">{salesChannel.description}</span>
        </div>
      </div>
      {isDisabled && (
      ...
      <div className={clsx('transition-all duration-200', classes)}>
        <div className="h-[55px]">
          <div className="mb-1 flex items-center justify-between">
            <h2 className="text-xlarge text-grey-90 font-semibold">{t('pages-sales-channels', 'Sales channels')}</h2>
...
function SalesChannelDetailsHeader(props: SalesChannelDetailsHeaderProps) {
  const { isDefault, salesChannel, openUpdateModal, resetDetails, showProductsAdd } = props;
  const { t } = useTranslation();

  const { mutate: deleteSalesChannel } = useAdminDeleteSalesChannel(salesChannel.id);

  const { mutate: updateSalesChannel } = useAdminUpdateSalesChannel(salesChannel.id);
  ...
  return (
    <div className="flex items-center justify-between">
      <h2 className="text-xlarge text-grey-90 mb-4 font-semibold">{salesChannel.name}</h2>
      <div className="flex items-center justify-between gap-4">
        <StatusSelector
...
function SalesChannelDetails(props: SalesChannelDetailsProps) {
  ...
  const [showUpdateModal, openUpdateModal, closeUpdateModal] = useToggleState(false);
  const [showAddProducts, showProductsAdd, hideProductsAdd] = useToggleState(false);
  ...
  return (
    <div className="rounded-rounded bg-grey-0 border-grey-20 col-span-2 col-span-2 h-fit border px-8 py-6">
      ...
      <SalesChannelProductsTable salesChannelId={salesChannel.id} showAddModal={showProductsAdd} />
...
function Details() {
  ...
  const [activeSalesChannel, setActiveSalesChannel] = useState<SalesChannel | null>();
  ...
  useEffect(() => {
    if (routeSalesChannelId !== activeSalesChannel?.id) {
      const activeChannel = sales_channels?.find(sc => sc.id === routeSalesChannelId);
  ...
  function filterSalesChannels(channels: SalesChannel[]) {
    ...
    return channels.filter(ch => {
      ...
      return !!ch.name.toLowerCase().match(filter) || !!ch.description?.toLowerCase().match(filter);
    });
  }
    ...
    <div>
      ...
      <div className="gap-x-xsmall grid grid-cols-3">
        <SalesChannelsList
          ...
          salesChannels={filterSalesChannels(sales_channels).sort(defaultChannelsSorter)}
        />
        {activeSalesChannel && (
          <SalesChannelDetails
            isDefault={activeSalesChannel.id === store?.default_sales_channel_id}
            salesChannel={sales_channels.find(sc => sc.id === activeSalesChannel.id)!}
            resetDetails={resetDetails}
          />
        )}
      </div>

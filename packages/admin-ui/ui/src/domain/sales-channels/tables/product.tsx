 ...
 */
export const ProductTable = forwardRef((props: ProductTableProps, ref: React.Ref<any>) => {
  const {
    tableActions,
          ...
          {rows.map(row => {
            ...
            return (
              <ProductRow
                ...
                disabled={!!row.original.sales_channels.find(sc => sc.id === currentSalesChannelId)}
                row={row}
    ...
    >
      <div className="shadow-toaster pointer-events-auto flex h-[48px] min-w-[224px] items-center justify-around gap-3 rounded-lg border px-4 py-3">
        <span className="text-small text-grey-50">
          {t('sales-channels-table-placeholder-selected-with-counts', '{{count}}', { count: total })}
        </span>
...
function SalesChannelProductsTable(props: SalesChannelProductsTableProps) {
  ...
  const { mutate: deleteProductsFromSalesChannel } = useAdminDeleteProductsFromSalesChannel(salesChannelId);
 ...
 * in a modal.
 */
function SalesChannelProductsSelectModal(props: SalesChannelProductsSelectModalProps) {

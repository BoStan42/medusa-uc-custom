...
type QueryObject = Partial<Pick<FilterState, 'q'>> & Pick<FilterState, 'limit' | 'offset'> & Record<string, string>;
 ...
 */
function parseQueryString<T>(queryString: string, defaults: Partial<FilterState>): FilterState {
...
const useQueryFilters = (defaultFilters: Partial<FilterState>) => {
  ...
  const [state, dispatch] = useReducer(reducer, parseQueryString(searchString, defaultFilters));
  ...
  const paginate = (direction: 1 | -1 | number, directionType?: 'goToPage') => {
    if (directionType === 'goToPage') {
  ...
  const getQueryString = () => qs.stringify(getQueryObject(state), { skipNulls: true });
  ...
  const representationObject = useMemo(() => getRepresentationObject(state), [state]);

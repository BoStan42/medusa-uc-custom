...
const allowedFilters = ['status', 'is_dynamic', 'q', 'created_at', 'offset', 'limit'];
...
const reducer = (state: PromotionFilterState, action: PromotionFilterAction): PromotionFilterState => {
  switch (action.type) {
    case 'setFilters': {
      return {
        ...state,
...
export const usePromotionFilters = (existing?: string, defaultFilters: PromotionDefaultFilters | null = null) => {
  ...
  const getQueryObject = () => {
    ...
    for (const [key, value] of Object.entries(state)) {
      if (key === 'query') {
        ...
        if (key === 'date') {
          toQuery[stateFilterMap[key]] = formatDateFilter(value.filter as PromotionDateFilter);

...
const AddProductsTable: React.FC<AddProductsTableProps> = ({ existingRelations, onSubmit, onClose }) => {
  ...
  useEffect(() => {
    setSelectedProducts(selectedProducts =>
      [
        ...selectedProducts.filter(sv => Object.keys(selectedRowIds).findIndex(id => id === sv.id) > -1),
        ...(products?.filter(
          p =>
    ...
    setRemovedProducts([
      ...existingRelations.filter(ap => Object.keys(selectedRowIds).findIndex(id => id === ap.id) < 0),
    ]);
  }, [selectedRowIds]);

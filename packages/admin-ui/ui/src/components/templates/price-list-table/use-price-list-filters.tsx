...
const reducer = (state: PriceListFilterState, action: PriceListFilterAction): PriceListFilterState => {
  switch (action.type) {
    case 'setFilters': {
      return {
        ...state,
...
export const usePriceListFilters = (existing?: string, defaultFilters: PriceListDefaultFilters | null = null) => {
      ...
      for (const [filter, value] of Object.entries(conditions)) {
        if (filter in clean) {
          if (Array.isArray(value)) {
            match = Array.isArray(clean[filter]) && setsEqual(new Set(clean[filter]), new Set(value));
          } else {

...
const reducer = (state: InventoryFilterState, action: InventoryFilterAction): InventoryFilterState => {
  switch (action.type) {
    case 'setFilters': {
      return {
        ...state,
...
export const useInventoryFilters = (existing?: string, defaultFilters: InventoryDefaultFilters | null = null) => {
  ...
  const getQueryObject = () => {
    ...
    for (const [key, value] of Object.entries(state)) {
      if (key === 'query') {
        ...
        if (key === 'date') {
          toQuery[stateFilterMap[key]] = formatDateFilter(value.filter as InventoryDateFilter);
        } else {
  ...
  const saveTab = (tabName: string, filters: InventoryFilterState) => {
    ...
    setTabs(prev => {
      const duplicate = prev.findIndex(prev => prev.label?.toLowerCase() === tabName.toLowerCase());
      if (duplicate !== -1) {

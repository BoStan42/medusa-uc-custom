...
import { InventoryItemDTO, InventoryLevelDTO, ProductVariant } from '@medusajs/medusa';
...
const InventoryTable: React.FC<InventoryTableProps> = () => {
  ...
  const { stock_locations, isLoading: locationsLoading } = useAdminStockLocations();
  ...
  return (
    ...
    >
      <Table
        ...
        tableActions={
          <LocationDropdown
            selectedLocation={queryObject.location_id || store?.default_location_id}
            onChange={id => {
            ...
            return (
              <Table.HeadRow key={key} {...rest}>
                {headerGroup.headers.map(col => {
                  ...
                  return (
                    <Table.HeadCell className="min-w-[100px]" key={key} {...rest}>
                      {col.render('Header')}
                    </Table.HeadCell>
                  );
                })}
...
const InventoryRow = ({
  ...
  const getRowActionables = () => {
    ...
    const actions = [
      {
        label: t('inventory-table-actions-adjust-availability', 'Adjust Availability'),
  ...
  return (
    ...
    >
      ...
      {isShowingAdjustAvailabilityModal && (
        <AdjustAvailabilityModal inventory={inventory} handleClose={closeAdjustAvailabilityModal} />
      )}
    </Table.Row>
  );
};
...
const AdjustAvailabilityModal = ({
  ...
  const { mutate: updateLocationLevelForInventoryItem, isLoading: isSubmitting } = useAdminUpdateLocationLevel(
    inventory.id!,
  );

  const notification = useNotification();

  const [stockedQuantity, setStockedQuantity] = useState(locationLevel?.stocked_quantity || 0);

  const disableSubmit = stockedQuantity === (locationLevel?.stocked_quantity || 0) || !variant || !locationLevel;
    ...
    <Modal handleClose={handleClose}>
                ...
                <span className="pr-base">
                  <div className="float-left my-1.5 mr-4 flex h-[40px] w-[30px] items-center">
                    {variant?.product?.thumbnail ? (
                      <img src={variant?.product?.thumbnail} className="rounded-rounded h-full object-cover" />
                    ) : (
                      <ImagePlaceholder />
                    )}
                  </div>
                  <div className="flex flex-col">
                    <span className="truncate">
                      ...
                      <span className="text-grey-50 truncate">({inventory.sku})</span>
                    </span>
      ...
      <Modal.Footer>
        <div className="gap-x-xsmall flex w-full justify-end">
          <Button size="small" variant="ghost" className="border" onClick={handleClose}>
            {t('inventory-table-cancel', 'Cancel')}
          </Button>
          <Button size="small" variant="primary" disabled={disableSubmit} loading={isSubmitting} onClick={onSubmit}>

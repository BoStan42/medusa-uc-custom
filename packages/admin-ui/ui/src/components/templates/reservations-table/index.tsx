...
import { useAdminDeleteReservation, useAdminReservations, useAdminStockLocations, useAdminStore } from 'medusa-react';
...
const LocationDropdown = ({
  ...
  const selectedLocObj = useMemo(() => {
    if (locationOptions?.length) {
      return (
        locationOptions.find((l: { value?: string; label: string }) => l.value === selectedLocation) ??
        locationOptions[0]
      );
    }
  }, [selectedLocation, locationOptions]);

  const isEllipsisActive = (e: { offsetWidth: number; scrollWidth: number } | null) => {
    if (!e) {
      return false;
    }
          ...
          {locationOptions.map((o, i) => (
            ...
            >
              <div className="mr-2 h-[20px] w-[20px]">
                {selectedLocObj?.value === o.value && <TagDotIcon size={20} outerColor="#FFF" color="#111827" />}
              </div>
        ...
        <Table
          ...
          tableActions={
            <div className="flex gap-2">
              <ReservationsFilters submitFilters={setFilters} clearFilters={setDefaultFilters} filters={filters} />
              <LocationDropdown
...
const ReservationRow = ({
  ...
  const [showEditReservation, setShowEditReservation] = useState<ReservationItemDTO | null>(null);

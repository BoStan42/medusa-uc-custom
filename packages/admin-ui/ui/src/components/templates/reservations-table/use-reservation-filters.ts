import { DateComparisonOperator, NumericalComparisonOperator, StringComparisonOperator } from '@medusajs/types';
...
const reducer = (state: ReservationFilterState, action: reservationFilterAction): ReservationFilterState => {
  switch (action.type) {
    case 'setFilters': {
      return {
        ...state,
...
export const useReservationFilters = (existing?: string, defaultFilters: ReservationDefaultFilters = {}) => {
  ...
  const getQueryObject = () => {
    ...
    for (const [key, value] of Object.entries(state)) {
      if (key === 'query') {
        ...
        if (key === 'date') {
          toQuery[stateFilterMap[key]] = formatDateFilter(value.filter as ReservationDateFilter);
        } else {
    ...
    for (const [key, value] of Object.entries(filters)) {
      if (allowedFilters.includes(key) && !!value) {
        switch (key) {
          ...
          case 'quantity': {
            defaultVal.additionalFilters.quantity = value as NumericalComparisonOperator;
            break;
          }
          case 'inventory_item_id':
          case 'created_by': {
            defaultVal.additionalFilters[key] = (Array.isArray(value) ? value : [value]) as string[];
            break;
          }
          case 'created_at': {
            defaultVal.additionalFilters.created_at = value as DateComparisonOperator;
            break;
          }
          default: {
            break;

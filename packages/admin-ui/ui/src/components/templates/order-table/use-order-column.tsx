...
const useOrderTableColums = () => {
  ...
  const decideStatus = status => {
    switch (status) {
      ...
      case 'refunded':
        return <StatusDot title={t('templates-refunded', 'Refunded')} variant="danger" />;
      case 'partially_refunded':
        return <StatusDot title={t('templates-partially-refunded', 'Partially Refunded')} variant="primary" />;
      default:
        ...
        Cell: ({ cell: { value } }) => {
          return (
            <div className={value?._odoo_order_create?._errors?.length ? 'text-red-600' : ''}>
              <span title="Sales Order">
                {value?._odoo_order_create?._odoo_order_id ? 'S' : <span className="text-gray-400">-</span>}
              </span>
              <span className="text-gray-400">/</span>
              <span title="Delivery Order">
                {value?._odoo_order_create?._odoo_delivery_order_id ? 'D' : <span className="text-gray-400">-</span>}
              </span>
            </div>
          );
        },

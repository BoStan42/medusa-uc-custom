...
const SalesChannelTable = ({
  ...
  return (
    <TableContainer
      ...
      isLoading={isLoading}
    >
      <Table {...getTableProps()} enableSearch handleSearch={setQuery} tableActions={tableAction}>
        <Table.Head>
    ...
    <div className="space-x-xsmall flex h-[34px] overflow-hidden">
      <div className={clsx('transition-all duration-200', classes)}>
        <div className="mb-2 flex h-[34px] items-center divide-x">
          <span className="inter-small-regular text-grey-50 mr-3">{numberOfSelectedRows} selected</span>
          <div className="space-x-xsmall flex pl-3">
            <Button onClick={onDeselect} size="small" variant="ghost" className="border-grey-20 border">
              Deselect
            </Button>
            <Button onClick={onRemove} size="small" variant="ghost" className="border-grey-20 border text-rose-50">
              Remove
            </Button>
          </div>
        </div>

...
const Timeline: React.FC<TimelineProps> = ({ orderId }) => {
  ...
  const { state: showRegisterClaim, close: closeRegisterClaim, open: openRegisterClaim } = useToggleState();
  ...
  const handleCreateNote = (value: string | undefined) => {
    ...
    createNote.mutate(
      ...
      {
        onSuccess: () =>
          notification(t('timeline-success', 'Success'), t('timeline-added-note', 'Added note'), 'success'),
        onError: err => notification(t('timeline-error', 'Error'), getErrorMessage(err), 'error'),
      },
    );
  };
